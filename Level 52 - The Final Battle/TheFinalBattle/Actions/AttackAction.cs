...
public class AttackAction : IAction
{
    ...
    public void Execute(Battle battle)
    {
        ...
        foreach (IAttackModifier modifier in _attacker.AllCapabilities.OffensiveModifiers)
            modifiedData = modifier.Modify(modifiedData, battle, _attacker, _target);
        foreach (IAttackModifier modifier in _target.AllCapabilities.DefensiveModifiers)
            modifiedData = modifier.Modify(modifiedData, battle, _attacker, _target);
        ...
        // Allow attack side effects to happen.
        foreach(IAttackSideEffect sideEffect in _attack.SideEffects)
            sideEffect.Perform(battle, _attacker, _attack, _target, initialData, modifiedData);
